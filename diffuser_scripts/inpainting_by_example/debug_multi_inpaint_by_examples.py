...
for init_label in range(num_init):
    ...
    for _ in range(num_img):
        ...
        for n in range(num_items):
            ...
            labels.append([folder_name, int(xmin), int(ymin), int(xmax), int(ymax)])
        ...
        for _, label in enumerate(labels):
            cv2.rectangle(open_cv_image, (label[1], label[2]), (label[3], label[4]), (255, 255, 0), 4) # (x, y)
            cv2.putText(open_cv_image, label[0],
                        (label[1] , label[2] - 10),
